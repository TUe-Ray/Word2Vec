...
The figure captures the practical impression from that stage: the space did not look cleanly organized enough to trust semantic nearest neighbors.
...
What `model_3` shows is not that the problem is fully solved, but that the failure mode changed. The later pipeline reduced the earlier collapse-like behavior, yet it still did not produce consistently clean semantic neighborhoods. That makes `model_3` a better endpoint than the earlier runs, but also a reminder that qualitative analysis is still necessary.
...
- replace qualitative inspection with one automatic metric or one final checkpoint number
